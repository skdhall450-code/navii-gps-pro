...
import { toPunjabCitySlug, type PunjabCitySeo } from "@/lib/seo/punjabCities";

const cityFaqs = (city: PunjabCitySeo) => [
  {
...
export function PunjabCityGpsPage({ city }: { city: PunjabCitySeo }) {
  const url = `https://naviigps.com/gps-tracker/punjab/${city.districtSlug}/${city.slug}`;
  const districtUrl = `https://naviigps.com/gps-tracker/punjab/${city.districtSlug}`;
    ...
    "@graph": [
      {
        ...
        description: `Vehicle GPS tracking and fleet management planning for ${city.name} in Punjab.`,
      ...
      },
      {
        ...
        areaServed: {
          "@type": "City",
          name: city.name,
          containedInPlace: { "@type": "AdministrativeArea", name: `${city.districtName} District, Punjab` },
        },
        url,
      },
      {
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Punjab", item: "https://naviigps.com/gps-tracker/punjab" },
  ...
  return <><Header /><main>
    ...
    <section className="bg-gradient-to-br from-[#041225] via-[#08224A] to-[#103B82] py-24 text-white md:py-32"><div className="mx-auto max-w-7xl px-6"><nav aria-label="Breadcrumb" className="mb-8 flex flex-wrap gap-3 text-sm text-cyan-200"><Link href="/">Home</Link><span>/</span><Link href="/gps-tracker/punjab">Punjab</Link><span>/</span><Link href={`/gps-tracker/punjab/${city.districtSlug}`}>{city.districtName}</Link><span>/</span><span>{city.name}</span></nav><span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-5 py-2 text-sm font-semibold tracking-[0.12em] text-cyan-200"><MapPin size={17} /> {city.districtName.toUpperCase()} CITY COVERAGE</span><h1 className="mt-7 max-w-5xl text-5xl font-extrabold leading-tight md:text-6xl">GPS Tracker in {city.name}, Punjab</h1><p className="mt-7 max-w-3xl text-lg leading-8 text-slate-200 md:text-xl">Vehicle tracking devices and fleet management software planning for cars, trucks, buses and commercial operations around {city.name} in {city.districtName} district.</p><div className="mt-10 flex flex-wrap gap-4"><Link href="/contact" className="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-7 py-4 font-semibold">Request a Consultation <ArrowRight size={19} /></Link><Link href="/products" className="rounded-xl border border-white/20 bg-white/5 px-7 py-4 font-semibold">Compare GPS Devices</Link></div></div></section>
