...
import type { DelhiDistrictSeo } from "@/lib/seo/delhiDistricts";
import { getDelhiAreasForDistrict } from "@/lib/seo/delhiAreas";

const districtFaqs = (district: DelhiDistrictSeo) => [
  {
    question: `Which vehicles can use GPS tracking in ${district.name} district, Delhi?`,
  ...
  {
    question: `Which areas are covered around ${district.name} district?`,
    answer: `Planning can include ${district.areas.join(", ")} and connected routes across Delhi NCR. Network and installation availability are confirmed for the actual operating locations.`,
  },
  {
    question: "Which fleet tracking features can be discussed?",
...
export function DelhiDistrictGpsPage({ district }: { district: DelhiDistrictSeo }) {
    ...
    "@graph": [
      {
        ...
        "@id": `${url}#webpage`,
        url,
        name: `GPS Tracker in ${district.name} District, Delhi | NAVII GPS`,
        description: `Vehicle GPS tracking and fleet management planning for ${district.name} district in Delhi.`,
        isPartOf: { "@id": "https://naviigps.com/#website" },
        inLanguage: "en-IN",
      },
      {
        "@type": "Service",
        name: `GPS Tracking Solutions in ${district.name} District, Delhi`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: "Delhi", item: "https://naviigps.com/gps-tracker/delhi" },
  ...
  return <><Header /><main>
    ...
    <section className="bg-slate-50 py-24"><div className="mx-auto grid max-w-6xl gap-14 px-6 lg:grid-cols-2"><div><p className="text-sm font-semibold tracking-[0.16em] text-blue-700">DISTRICT FLEET PLANNING</p><h2 className="mt-4 text-4xl font-extrabold text-slate-900">Vehicle Tracking Across {district.name} Delhi</h2><p className="mt-6 text-lg leading-8 text-slate-600">{district.localContext}</p><p className="mt-4 leading-8 text-slate-600">{district.planningNote}</p><ul className="mt-8 space-y-4">{district.sectors.map((sector) => <li key={sector} className="flex items-start gap-3 capitalize text-slate-700"><CheckCircle2 className="mt-0.5 shrink-0 text-emerald-500" size={20} />{sector}</li>)}</ul></div><div className="rounded-[32px] bg-[#06142E] p-9 text-white shadow-xl"><h2 className="text-3xl font-bold">Areas in {district.name} District</h2><p className="mt-5 leading-8 text-slate-300">Choose an area for its dedicated vehicle tracking guide, installation considerations and connected fleet routes.</p><div className="mt-7 grid gap-3">{areaGuides.map((area) => <Link key={area.slug} href={`/gps-tracker/delhi/${district.slug}/${area.slug}`} className="rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-3 text-cyan-100 hover:border-cyan-300">GPS Tracker in {area.name}</Link>)}</div><Link href="/gps-tracker/delhi" className="mt-8 inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-3 font-semibold">View All Delhi Districts <ArrowRight size={17} /></Link></div></div></section>
    <section className="bg-white py-20"><div className="mx-auto max-w-5xl px-6"><h2 className="text-center text-4xl font-extrabold text-slate-900">GPS Tracking in {district.name} Delhi: FAQs</h2><div className="mt-10 space-y-4">{faqs.map((faq) => <article key={faq.question} className="rounded-2xl border border-slate-200 p-6"><h3 className="text-lg font-bold text-slate-900">{faq.question}</h3><p className="mt-3 leading-7 text-slate-600">{faq.answer}</p></article>)}</div></div></section>
