...
export default async function StateGpsTrackerPage({ params }: PageProps) {
  ...
  const districtGuides = state.slug === "haryana" ? haryanaDistricts : [];
  const url = `https://naviigps.com/gps-tracker/${state.slug}`;
  ...
  return <><Header /><main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData).replace(/</g, "\\u003c") }} />
    ...
    {districtGuides.length > 0 && <section className="bg-slate-50 py-20"><div className="mx-auto max-w-7xl px-6"><p className="text-sm font-semibold tracking-[0.16em] text-blue-700">DISTRICT-WISE COVERAGE</p><h2 className="mt-3 text-4xl font-extrabold text-slate-900">GPS Tracker Guides for All 23 Haryana Districts</h2><p className="mt-4 max-w-3xl leading-7 text-slate-600">Choose a district to review local city and town keywords, fleet use cases, route context and deployment considerations.</p><div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{districtGuides.map((district) => <Link key={district.slug} href={`/gps-tracker/haryana/${district.slug}`} className="rounded-2xl border border-slate-200 bg-white p-5 text-blue-800 shadow-sm transition hover:border-cyan-500"><h3 className="font-bold">{district.name} District</h3><p className="mt-2 text-sm text-slate-600">{district.cities.slice(0, 3).join(", ")}</p></Link>)}</div></div></section>}
