...
const dataSource = readFileSync("lib/seo/punjabDistricts.ts", "utf8");
...
assert.equal(districts.length, 23, "Expected all 23 Punjab district records");
assert.deepEqual(districts.map((district) => district.slug).sort(), expectedSlugs.sort());
assert.equal(new Set(districts.map((district) => district.slug)).size, 23, "Duplicate Punjab district slugs");
assert.equal(new Set(districts.map((district) => district.localContext)).size, 23, "Duplicate Punjab district content");
assert.equal(new Set(districts.map((district) => district.planningNote)).size, 23, "Duplicate Punjab district guidance");
...
assert.ok(routeSource.includes("punjabDistricts.map"), "Automatic Punjab district static params are missing");
assert.ok(routeSource.includes("generatePunjabDistrictMetadata"), "Automatic Punjab district metadata is missing");
assert.ok(routeSource.includes("PunjabDistrictGpsPage"), "Punjab district page routing is missing");
assert.ok(stateSource.includes('state.slug === "punjab"'), "Punjab state-to-district data is missing");
assert.ok(stateSource.includes("`/gps-tracker/${state.slug}/${district.slug}`"), "Dynamic state-to-district links are missing");
assert.ok(sitemapSource.includes("punjabDistrictRoutes"), "Automatic Punjab district sitemap routes are missing");
assert.ok(dataSource.includes("district.cities.flatMap"), "Automatic Punjab city keyword generation is missing");

if (process.argv.includes("--source-only")) {
  console.log("PASS: 23 Punjab districts with city keywords, unique content and automatic routes verified.");
  process.exit(0);
}

const root = ".next/server/app";
const sitemap = readFileSync(`${root}/sitemap.xml.body`, "utf8");
const stateHtml = readFileSync(`${root}/gps-tracker/punjab.html`, "utf8");
const sitemapUrls = [...sitemap.matchAll(/<loc>(.*?)<\/loc>/g)].map((match) => match[1]);
assert.equal(sitemapUrls.length, new Set(sitemapUrls).size, "Duplicate sitemap URLs");

for (const district of districts) {
  const route = `/gps-tracker/punjab/${district.slug}`;
  const canonical = `https://naviigps.com${route}`;
  const htmlPath = `${root}${route}.html`;
  assert.ok(existsSync(htmlPath), `Missing rendered Punjab district page: ${route}`);
  const html = readFileSync(htmlPath, "utf8");
  assert.ok(html.includes(`rel="canonical" href="${canonical}"`), `Invalid district canonical: ${route}`);
  assert.ok(
    html.includes(`<title>GPS Tracker in ${district.name} District, Punjab | NAVII GPS INDIA</title>`),
    `Invalid district title: ${route}`,
  );
  ...
  assert.ok(stateHtml.includes(`href="${route}"`), `District orphaned from Punjab state page: ${route}`);
  assert.ok(html.includes('href="/gps-tracker/punjab"'), `District missing parent state link: ${route}`);
...
console.log("PASS: 23 rendered Punjab district pages; city keywords, canonical, schema, sitemap and state links verified.");
